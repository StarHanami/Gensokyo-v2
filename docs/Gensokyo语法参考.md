...
| 成员变动 | \[CQ:member,type=add/remove,group_id=虚拟群ID,user_id=虚拟用户ID] | 🏷️ 群聊 | 群成员入群/退群 CQ 码。[详细](./cq码/扩展CQ码/cq-member.md) |
| 主动标记 | \[CQ:active] | 🌐 全场景 | 标记主动推送模式。[详细](./cq码/扩展CQ码/cq-active.md) |
| Markdown @ | \[CQ:at,qq=虚拟用户ID]（Markdown 上下文） | 🏷️ 群聊 | Markdown 消息中 @ 用户，自动合并到 md 内容。[详细](./cq码/扩展CQ码/cq-at-md.md) |
