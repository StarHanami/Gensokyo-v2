...
| Action | 文件 | 说明 |
|--------|------|------|
| `send_msg` | send_msg.go | 发送消息（自动判断群聊/私聊/频道） |
...
| Action | 文件 | 说明 |
|--------|------|------|
| `send_guild_channel_msg` | send_guild_channel_msg.go | 发送频道消�?|
| `send_guild_channel_forum` | send_guild_channel_forum.go | 发送频道帖�?|
...
| `get_guild_channel_list` | get_guild_channel_list.go | 获取频道子频道列�?|
...
| `get_robot_share_link` | [get_robot_share_link.go](./扩展api/扩展api-get_robot_share_link.md) | 获取机器人分享链�?|
| `put_interaction` | [put_interaction.go](./扩展api/扩展api-put_interaction.md) | 处理按钮交互回调 |
| `send_private_msg_wakeup` | [send_private_msg_wakeup.go](./扩展api/扩展api-send_private_msg_wakeup.md) | 发送被动唤醒私聊消�?|
...
Gensokyo 在群消息事件中增加了 `to_me` 字段，标识消息是�?@ 了机器人�?
...
**取值逻辑�?*
- `GROUP_AT_MESSAGE_CREATE`（@消息事件）→ `to_me: true` **始终�?true**
- `GROUP_MESSAGE_CREATE`（普通群消息事件）→ 检�?`Mentions` 中是否有 `IsYou=true`，有�?`true`，否�?`false`
...
�?`GROUP_MESSAGE_CREATE`（普通群消息）中，若内容包含对其他用�?机器人的 `@`，Gensokyo 会自动将原始 `<@OpenID>` 替换为标准的 `[CQ:at,qq=虚拟ID]` 格式，并建立 OpenID �?虚拟 ID 的映射，确保同一用户多次出现�?@ 虚拟 ID 始终一致�?
...
出站（后�?�?QQ API�?
  后端发�? [CQ:at,qq=713011248] 贴贴
  QQ API:   <@!B1FE88...> 贴贴
```

**规则�?*
- 入站 `@bot` �?�?content 中剥离（`to_me = true`�?
- 入站 `@其他人` �?转为 `[CQ:at,qq=虚拟ID]`
- 出站 `[CQ:at,qq=数字]` �?转为 `<qqbot-at-user id="OpenID" />`（最�?QQ 格式，频道场景渲染为蓝色 @�?
- 群聊纯文本出站消息中 @ 不渲染（API 限制�?

### Markdown 中的 @ 能力

�?Markdown 卡片（`msg_type=2`）内容中嵌入 `[CQ:at,qq=数字]`，Gensokyo 会自动将其转换为 QQ API �?`<qqbot-at-user>` 标签，在群聊和频道中渲染为蓝�?@�?
...
Gensokyo 在群消息和私聊消息中，当 `card_nick` 配置为空时，会自动从 QQ API 返回�?`data.Author.Username` 中提取用户名填充�?`event.sender.nickname`�?

```json
{
    "sender": {
        "nickname": "NIKO",   // �?自动�?Author.Username 获取
        "user_id": 123456789,
        ...
    }
}
...
优先顺序：`card_nick` 配置�?> `Author.Username` > �?

### Sender.Role 身份标识

群消息中 `sender.role` 字段会优先使�?QQ API 返回�?`member_role`，不再仅依赖 `master_id` 配置�?
...
API 未返回时回退�?`master_id` 配置判断，保持兼容。详见[本版新增功能](./本版新增功能.md#群消�?senderrole-支持-member_role)�?
```
