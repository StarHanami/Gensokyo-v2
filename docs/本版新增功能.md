...
出站 `[CQ:at,qq=<虚拟ID>]` 会反查 OpenID，并转换为：

```text
<qqbot-at-user id="OpenID" />
```

Markdown 内容里的 `[CQ:at]` 也会在发送前转换。详见 [Markdown 中的 CQ at](./cq码/扩展CQ码/扩展cq码-cq-at-md.md)。
...
- 文件日志按天、按级别写入。
- 可选 JSON 文件日志：`log_json_output`。
- 单文件大小轮转：`log_max_size_mb`。
- 历史日志压缩和清理：`log_max_age_days`。
...
| 指令 | 行为 |
|------|------|
| `-status` | 返回 uptime、内存、协程数、收发计数、错误计数、慢事件计数。 |
| `-broadcast <内容>` | 向已发现的 q頻文本子频道和 idmap 中的 q群提交广播。 |
