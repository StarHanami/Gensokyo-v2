...
`data` 是经过 base64 编码的 JSON 数据，支持与其他 CQ 码拼接。

### Message Segment（数组/对象）
...
data 支持传 `string`（纯文本）、`base64://` 编码的 JSON，或 `map` 对象（自动序列化）。
...
内部结构为 `data.data.markdown` + `data.data.keyboard` 双层嵌套。

### 直接调用 API

若直调 QQ API，msg_type 设为 2：
      ...
      "rows": [
        {
          "buttons": [
            {
              "render_data": { "label": "再来一次", "visited_label": "再来一次" },
              "action": {
                "type": 1,
